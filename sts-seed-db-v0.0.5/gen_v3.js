// STS Seed DB - v3: 正确的 STS1 种子编码 (35进制, 无O)
const fs = require('fs');
const path = require('path');

// ===== STS1 种子编码 (来自 SeedHelper.java 反编译) =====
// 字符集: "0123456789ABCDEFGHIJKLMNPQRSTUVWXYZ" (35字符, 字母O被替换为0)
const STS1_CHARS = '0123456789ABCDEFGHIJKLMNPQRSTUVWXYZ';
const STS1_BASE = BigInt(STS1_CHARS.length); // 35
...
console.log('STS1:', sts1Seeds.length, '| Hu:', sts1Hu.length, '| Du:', sts1Du.length);
console.log('STS2:', sts2Seeds.length, '| Hu:', sts2Hu.length, '| Du:', sts2Du.length);
console.log('Sample STS1 seeds:');
sts1Seeds.slice(0, 5).forEach(s => console.log(' ', s.seed, '|', s.char, '| floor:', s.floor, '| win:', s.victory));
console.log('Sample STS2 seeds:');
sts2Seeds.slice(0, 3).forEach(s => console.log(' ', s.seed, '|', s.char, '| win:', s.victory));
...
.seed-card{background:#16213e;border-radius:10px;padding:14px;border:1px solid #2a2a4a;transition:border-color .2s;}
.seed-card:hover{border-color:#4fc3f7;}
...
.seed-num{font-family:monospace;font-size:15px;color:#4fc3f7;word-break:break-all;flex:1;letter-spacing:1px;}
...
.seed-game{font-size:11px;color:#555;margin-bottom:4px;}
...
<body>
<h1>🎴 STS 种子库</h1>
<p class="subtitle">杀戮尖塔 1 &amp; 2 · 存档种子管理 · Beta v0.3</p>
...
<p class="note">STS1 种子编码: 35进制 (0-9 A-N P-Z, 无O) · 可直接在游戏种子输入框使用</p>
...
function makeCard(s) {
    ...
    return '<div class="seed-card"><div class="seed-header"><span class="char-badge">'+cl+'</span><span class="'+tc+'">'+tl+'</span>'+wt+'</div><div class="seed-game">'+s.game+'</div><div class="seed-seed"><span class="seed-num">'+s.seed+'</span><button class="btn-copy" onclick="copySeed(\\''+s.seed+'\\')">复制</button></div><div class="seed-meta">楼层: '+s.floor+'</div></div>';
}
